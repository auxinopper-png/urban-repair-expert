...
const STATS = [
  ...
  { value: parseFloat(SITE.ratingValue), suffix: "", label: "Google Rating", decimal: true },
  { value: 60, suffix: "s", label: "To Book Online" },
  { value: 180, suffix: "-Day", label: "Service Warranty" },
];
        ...
        <div className="grid grid-cols-2 gap-3 rounded-[28px] border border-slate-100 bg-white p-5 shadow-card sm:p-7 lg:grid-cols-4">
          {STATS.map((s) => (
            <div key={s.label} className="text-center">
              <p className="text-2xl font-extrabold tracking-tight text-brand-700 sm:text-3xl">
                {s.decimal ? (
                  <>
                    {s.value.toFixed(1)}
                    {s.suffix}
